...
当用户要求分析基金的投资价值时，AI 应进行**全面综合评估**，而不是简单的"好"或"不好"。
...
3. **归因分析**
   - 收益来源分解：Alpha + Beta + 行业配置 + 个股选择
   - 判断是运气还是实力（长期业绩是否稳定）
   - 分析超额收益的可持续性

4. **持仓分析**
   - 行业配置：是否分散、是否有明显偏好
   - 重仓股质量：基本面是否优秀、估值是否合理
   - 换手率：高换手（交易型）vs 低换手（价值型）
   - 集中度：是否过于集中
