...
class DataStore:
    ...
    def cache_price(self, asset_code: str, asset_type: str, data: Dict[str, Any],
                    market_type: str = None, source: str = None) -> bool:
        """
        缓存价格数据
        ...
        Args:
            asset_code: 资产代码
            asset_type: 资产类型 (stock, fund, etf, index, gold, crypto)
            data: 价格数据字典，包含 price, change_percent, volume 等
            market_type: 市场类型
            source: 数据来源
        ...
        try:
            with self._get_conn() as conn:
                ...
                cursor.execute("""
                    ...
                    (asset_code, asset_type, market_type, price, change_percent, volume, source, raw_data, timestamp)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
                """, (
                    asset_code,
                    asset_type,
            ...
            logger.error(f"Failed to cache price for {asset_code}: {e}")
            return False

    def get_cached_price(self, asset_code: str, asset_type: str,
        ...
        Args:
            asset_code: 资产代码
            asset_type: 资产类型
            max_age_seconds: 最大缓存年龄，None则使用默认配置

        Returns:
            缓存的数据或None
        """
        if max_age_seconds is None:
            max_age_seconds = self.config.price_ttl
        ...
        try:
            with self._get_conn() as conn:
                ...
                cursor.execute("""
                    SELECT raw_data, timestamp FROM price_cache
                    WHERE asset_code = ? AND asset_type = ?
                """, (asset_code, asset_type))
            ...
            logger.error(f"Failed to get cached price for {asset_code}: {e}")
            return None
    ...
    def get_price_history(self, asset_code: str, asset_type: str,
                         start_date: str = None, end_date: str = None,
        ...
        try:
            with self._get_conn(StorageType.CACHE) as conn:
                ...
                cursor.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
                """, (
                    asset_code, asset_type, market_type,
                    data.get('price'), data.get('change_percent'), data.get('volume'),
                    source, raw_data
                ))
                conn.commit()
                return True
    ...
    def get_cached_price(self, asset_code: str, asset_type: str,
        ...
        """获取缓存的实时价格"""
        if max_age_seconds is None:
            max_age_seconds = self.cache_config.price_ttl
